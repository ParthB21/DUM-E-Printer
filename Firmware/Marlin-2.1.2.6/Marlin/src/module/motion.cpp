...
void get_cartesian_from_steppers() {
  ...
  #elif IS_SCARA
    float arm_angles[3];
    if (!articulated_inverse_kinematics(
          cartesian_position[X_AXIS],
          cartesian_position[Y_AXIS],
          cartesian_position[Z_AXIS],
          arm_angles
        )) {
      return; // Reject unreachable or unsafe move
    }

    delta[X_AXIS] = arm_angles[0];
    delta[Y_AXIS] = arm_angles[1];
    delta[Z_AXIS] = arm_angles[2];
  #else
    NUM_AXIS_CODE(
