...
bool articulated_inverse_kinematics(
  ...
  // Reachability
  const float max_reach = ARM_L1 + ARM_L2;
  const float min_reach = fabsf(ARM_L1 - ARM_L2);
  ...
  const float D =
    (r*r + z*z - ARM_L1*ARM_L1 - ARM_L2*ARM_L2) /
    (2.0f * ARM_L1 * ARM_L2);
  ...
  // Elbow-down solution
  const float t3 = atan2f(-sqrtf(1.0f - D*D), D);

  // Shoulder
  const float t2 =
    atan2f(z, r) -
    atan2f(ARM_L2 * sinf(t3), ARM_L1 + ARM_L2 * cosf(t3));

  // Convert to degrees
  angles[0] = t1 * 180.0f / M_PI;
  angles[1] = t2 * 180.0f / M_PI;
  angles[2] = t3 * 180.0f / M_PI;

  // Joint limits
  if (angles[0] < J1_MIN || angles[0] > J1_MAX) return false;
  if (angles[1] < J2_MIN || angles[1] > J2_MAX) return false;
  if (angles[2] < J3_MIN || angles[2] > J3_MAX) return false;

  // Singularity protection (near-collinear)
  if (fabsf(ARM_L2 * sinf(t3)) < 10.0f)
    return false;
